...
class FileCache<T> {
  ...
  ///
  /// 임시 디렉토리 하위에 philgo_cache/{cacheName} 디렉토리 생성
  Future<Directory> _getCacheDirectory() async {
    final tempDir = await getTemporaryDirectory();
    final cacheDir = Directory('${tempDir.path}/philgo_cache/$cacheName');
  ...
  Future<T?> get(String key) async {
    ...
    if (useMemoryCache && _memoryCache.containsKey(key)) {
      ...
      if (!entry.isExpired) {
        debugPrint('FileCache[$cacheName]: 메모리 캐시 히트 - $key');
        return entry.data;
      }
      // 만료된 메모리 캐시 제거
    ...
    try {
      ...
      if (await file.exists()) {
        ...
        if (!entry.isExpired) {
          ...
          debugPrint('FileCache[$cacheName]: 파일 캐시 히트 - $key');
          return entry.data;
        }

        // 만료된 파일 삭제
        debugPrint('FileCache[$cacheName]: 캐시 만료, 파일 삭제 - $key');
        await file.delete();
      }
    } catch (e) {
      debugPrint('FileCache[$cacheName]: 캐시 로드 실패 - $key, $e');
    }
  ...
  Future<void> set(String key, T data, {Duration? ttl}) async {
    ...
    try {
      ...
      debugPrint('FileCache[$cacheName]: 캐시 저장 완료 - $key');
    } catch (e) {
      debugPrint('FileCache[$cacheName]: 캐시 저장 실패 - $key, $e');
    }
  }
  ...
  Future<void> remove(String key) async {
    ...
    try {
      ...
      if (await file.exists()) {
        await file.delete();
        debugPrint('FileCache[$cacheName]: 캐시 삭제 완료 - $key');
      }
    } catch (e) {
      debugPrint('FileCache[$cacheName]: 캐시 삭제 실패 - $key, $e');
    }
  }
  ...
  Future<void> clear() async {
    ...
    try {
      ...
      if (await cacheDir.exists()) {
        ...
        debugPrint('FileCache[$cacheName]: 전체 캐시 삭제 완료');
      }
    } catch (e) {
      debugPrint('FileCache[$cacheName]: 전체 캐시 삭제 실패 - $e');
    }
  }
  ...
  Future<void> cleanup() async {
    ...
    try {
      ...
      if (deletedCount > 0) {
        debugPrint('FileCache[$cacheName]: 만료된 캐시 $deletedCount개 정리 완료');
      }
    } catch (e) {
      debugPrint('FileCache[$cacheName]: 캐시 정리 실패 - $e');
    }
  }
