...
### 🔴🔴🔴 2.2 최우선 설계 원칙: 비즈니스 로직은 반드시 API 단(Service)에서 처리 🔴🔴🔴

> **⛔⛔⛔ 절대 규칙: 모든 비즈니스 로직(데이터 변환, 기간 제한, 콘텐츠 차단, 권한 검증, 유효성 검사 등)은
> 반드시 API 단(Service/Controller)에서 처리해야 한다. 웹뷰(v7/post/view.php 등)에서 처리하는 것은 금지한다. ⛔⛔⛔**
>
> **이유:** v7 API는 PHP 웹뷰뿐만 아니라 Flutter 앱, 외부 클라이언트 등 다양한 클라이언트에서 호출된다.
> 웹뷰에서만 비즈니스 로직을 처리하면, 다른 클라이언트에서는 해당 로직이 적용되지 않아 데이터 불일치가 발생한다.
>
> **올바른 예시:**
> - ✅ `PostService::applyExpiredJobPostPolicy()` — 구인구직 6개월 만료 처리를 Service에서 수행
> - ✅ `PostService::get()` 내부에서 데이터 변환/차단 로직 실행
> - ✅ 웹뷰는 Service가 반환한 결과를 렌더링하거나 UI 표시용 분기만 담당
>
> **잘못된 예시:**
> - ❌ `v7/post/view.php`에서 `if (6개월 전) { content = 안내메시지 }` — 웹에서만 적용, 앱에는 미적용
> - ❌ 웹뷰에서 권한 검증 후 데이터 숨김 — 앱에서는 모든 데이터가 노출됨

| 계층 | 역할 | SQL 직접 사용 | 비즈니스 로직 |
|------|------|:---:|:---:|
| **View (v7/*.php, v7/widgets/)** | Service 결과 렌더링, UI 전용 분기 | ⛔ 절대 금지 | ⛔ 금지 |
| **Controller** | 입력 파싱, 인증 확인, Service 호출 | ⛔ 금지 | ⛔ 금지 (Service 위임) |
| **Service** | 비즈니스 로직, 데이터 변환, 권한 검증, 유효성 검사 | ⛔ 금지 (Repository 위임) | ✅ 허용 (유일하게) |
| **Repository** | SQL 쿼리 실행, Entity 반환 | ✅ 허용 (유일하게) | ⛔ 금지 |
| **Entity** | 데이터 구조 정의, fromArray/toArray | ⛔ 금지 | ⛔ 금지 |

> **⛔ 뷰(v7/*.php)에서 `Db::fetch()`, `Db::fetchAll()`, `Db::execute()` 등 SQL 관련 코드를 직접 호출하는 것은 절대 금지한다.**
