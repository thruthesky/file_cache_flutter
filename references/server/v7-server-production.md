...
### cloudflare-realip.conf 전체 원본

파일 경로: `/etc/nginx/conf.d/cloudflare-realip.conf`

Cloudflare가 전달하는 실제 클라이언트 IP를 복원하기 위한 설정 파일이다. `nginx.conf`에서 `include /etc/nginx/conf.d/cloudflare-realip.conf;`로 로드된다.

```nginx
# --- Cloudflare Real IP (include from http {}) ---
