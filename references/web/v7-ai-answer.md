...
- `ai.saveAnswer` API를 통해 저장하며, 마크다운 형식의 텍스트가 저장된다.
- 중복 저장 방지: `text_7`이 이미 존재하면 저장을 거부한다.
...
| `lib/ai/AiController.php` | `generateStream()`, `saveAnswer()`, `enforceAiApiEndpoint()` |
| `lib/ai/AiService.php` | `generateStream()`, `saveAnswer()`, `getPostAnswerSystemPrompt()` |
...
3. `fetch('/ai-api.php', { method: 'POST', body: ... })`로 SSE 요청을 보낸다.
...
8. 스트리밍 완료 시 `ai.saveAnswer` API를 호출하여 답변을 저장한다.

### 6.2. 서버 (PHP)

1. `AiController::generateStream()`이 호출된다.
2. `enforceAiApiEndpoint()`로 `ai-api.php` 경로를 검증한다.
3. SSE 헤더를 설정한다: `Content-Type: text/event-stream`, `X-Accel-Buffering: no`.
4. `AiService::generateStream()`을 호출한다.
5. 모델 화이트리스트를 검증한다.
6. 게시판 검증 (qna/freetalk만 허용)한다.
7. 중복 방지 (`text_7`이 이미 있으면 거부)한다.
8. `GeminiClient::generateContentStream()`을 호출한다.
9. cURL `WRITEFUNCTION` 콜백으로 Gemini API의 SSE 청크를 즉시 `echo` + `flush`한다.
10. 스트리밍 완료 후 `exit`으로 `api.php`의 JSON 응답을 우회한다.
...
| 코멘트 거부 | `idx_parent > 0`인 코멘트에는 AI 답변을 저장할 수 없음 |
