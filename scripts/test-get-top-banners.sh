...
# 상단 배너 API 테스트 스크립트
#
# 용도: get_top_banners API를 호출하여 상단 배너 목록을 조회합니다.
#
# 사용법:
#   ./test-get-top-banners.sh                  # 전체 상단 배너 조회
#   ./test-get-top-banners.sh wanted           # wanted 카테고리 배너 조회
#   ./test-get-top-banners.sh freetalk         # freetalk 카테고리 배너 조회
#
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}   상단 배너 API 테스트 (get_top_banners)${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""

# 요청 데이터 생성
if [ -z "$CATEGORY" ]; then
    REQUEST_DATA='{"func": "get_top_banners"}'
    echo -e "${YELLOW}카테고리:${NC} 없음 (전체 배너)"
else
    REQUEST_DATA="{\"func\": \"get_top_banners\", \"category\": \"$CATEGORY\"}"
...
if echo "$RESPONSE" | grep -q "cf_chl"; then
    ...
    echo ""
    echo "   func('get_top_banners', { category: '$CATEGORY' }).then(r => console.log(r));"
    echo ""
    exit 1
fi
