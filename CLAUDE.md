...
개발자가 "명칭"이라고만 요청할 경우:
- ✅ 소스 코드의 각 UI/위젯/항목별로 적절한 한글 명칭을 주석으로 기록
- ✅ 개발자 간 원활한 소통을 위한 공통 용어 정의
- ✅ 코드 그룹별로 이해하기 쉬운 설명 추가

**명칭 주석 작성 규칙**:
- 각 UI 요소나 위젯 그룹의 상단에 `///` 형식으로 명칭 주석 추가
- 명칭은 간결하고 명확하게 한글로 작성
- 해당 요소의 역할과 기능을 직관적으로 알 수 있도록 표현
...
개발자가 UI/디자인 관련 요청을 할 경우:
- ✅ **반드시 [앱 디자인 가이드](./docs/apps/app-design.md) 문서를 먼저 참고**
- ✅ Flat 디자인 원칙 준수 (테두리/그림자 금지)
- ✅ Theme 기반 스타일링 필수
- ✅ Font Awesome Pro 아이콘 사용 (Light > Regular > Solid 순)
- ✅ 8의 배수 간격 사용
- ✅ elevation은 0으로 설정
...
**중요**: 모든 디자인 작업은 app-design.md의 규칙을 따라야 하며, main.dart Theme 수정은 명시적 요청 시에만 가능합니다.
