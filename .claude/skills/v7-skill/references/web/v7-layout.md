...
| **보더 없는 디자인** | 카드, 입력창, 빈 상태 영역에 `border` 사용 금지. 영역 구분은 매우 연한 배경색(neutral-95)으로 한다. 배경색은 연하게(neutral-95 이상), 전경색은 진하게(neutral-40~50). 진한 배경색(neutral-80 이하) 절대 금지 |
| **Shadow 금지** | 기본 상태에서 `box-shadow` 사용 금지. hover 시에만 허용 |
...
### Web Awesome CSS 변수 사용 시 주의사항

> **⚠️ Web Awesome의 `--wa-color-brand-*`, `--wa-color-neutral-*` CSS 변수는 Tailwind CSS 색상 팔레트 번호와 동일하지 않다.**
> Web Awesome은 자체 테마 시스템에 따라 CSS 변수를 해석하므로, `--wa-color-brand-50`이 반드시 연한 파란색(`#eff6ff`)이 되는 것이 아니다.
> 의도한 색상과 실제 렌더링 결과가 다를 수 있으므로, **연한 배경색/텍스트 색상 등 정밀한 색상 제어가 필요한 경우에는 하드코딩 색상값을 직접 지정**하는 것이 안전하다.

| 상황 | 권장 방법 |
|------|----------|
| Web Awesome 컴포넌트(`wa-button`, `wa-input` 등)의 테마 색상 | `--wa-color-*` CSS 변수 사용 (Web Awesome이 올바르게 처리) |
| 커스텀 HTML 요소의 배경/텍스트/보더 색상 (정밀 제어 필요) | 하드코딩 색상값 직접 지정 |
| Web Awesome 변수를 사용했으나 렌더링 결과가 의도와 다른 경우 | 하드코딩 색상값으로 교체 |

**실제 사례**: 즐겨찾기 위젯(`.v7-bm-widget-*`)에서 `--wa-color-brand-50`을 배경색으로 사용했으나, 연한 파란색 대신 진한 테마 색상이 적용되어 아이콘/라벨 배경이 의도보다 진하게 표시되는 문제가 발생하여, 모든 색상을 하드코딩 값으로 교체했다. 상세 내용은 [v7-widgets.md 14.5절](v7-widgets.md#145-css-클래스) 참조.
