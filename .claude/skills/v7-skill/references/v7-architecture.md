...
| 계층 | 역할 | 금지 사항 |
|------|------|-----------|
| **Service** | 비즈니스 로직, 데이터 변환, 권한 검증, 유효성 검사 | 없음 (모든 로직은 여기서) |
| **Controller** | 입력 파싱, Service 호출, 결과 반환 | 복잡한 비즈니스 로직 직접 구현 |
| **웹뷰 (v7/*.php)** | Service 결과 렌더링, UI 전용 분기 | 비즈니스 로직, 데이터 변환, 권한 검증 |
